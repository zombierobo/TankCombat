...
var tank_width = 50*scaling_factor;
var tank_length = 65*scaling_factor;
var tank_color = 'grey' ;
...
var tank_x = canvas.width/2;
var tank_y = canvas.height/2;
var rotation = 30;
var tank_angle = 0;
var tank_gun_angle = 0;
...
function randomRendering(){
	...
	t1.set_tank_position({x :canvas.width/2,y:canvas.height/2});	
	t1.set_tank_angle(60);
	t1.set_gun_angle(0);
	t1.render();
	console.log('gun width : '+t1.get_gun_width() + ' gun length : '+t1.get_gun_length());
	console.log('gun tip : x = '+t1.get_gun_tip().x + ' ,y = '+t1.get_gun_tip().y);
...
function draw()
{
	...
	//tank_angle += rotation;
	tank_gun_angle -= rotation;
	//tank_x += 1;
	//tank_y += 1;
	t1.set_tank_position({x:tank_x,y:tank_y});	
	//t1.set_tank_angle(tank_angle);
	t1.set_gun_angle(tank_gun_angle);
	console.log('gun angle : ' + t1.get_gun_angle() + ' gun tip : x = '+t1.get_gun_tip().x + ' ,y = '+t1.get_gun_tip().y);
	ctx.closePath();
}
...
function drawVericalLine(x,y,length)
{
	length = length || 100;
	ctx.save();
	ctx.lineWidth = 10;
